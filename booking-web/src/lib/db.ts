...
export async function createBooking(
  ...
  const participantNamesJson = participantNames && participantNames.length > 0 
    ? JSON.stringify(participantNames) 
    : null;
    
  const result = await query(
    'INSERT INTO bookings (customer_name, customer_email, customer_phone, booking_date, participants, participant_names, notes) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
    [customerName, customerEmail, customerPhone, bookingDate, participants, participantNamesJson, notes || null] as any
  );
